...
void ifree(int inodeNumber)
{
	...
	for (i = 2; i < NO_OF_FILES_IN_DIR; i++) {
		if (AllDir[UA.current_working_directory].file[i].inode_number ==
		    inodeNumber) {
			if (DILB
			    [AllDir[UA.current_working_directory].file[i].
			     inode_number].type == Directory) {
				for (j = 0; j < NO_OF_DIR; j++) {
					if (AllDir[j].inode_number ==
					    inodeNumber) {
						entryInDir = j;
						break;
					}
			...
			AllDir[UA.current_working_directory].file[i].
			    inode_number = -1;
			memset(AllDir[UA.current_working_directory].file[i].
			       file_name, '\0', 20);
		}
	}
