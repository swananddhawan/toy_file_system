...
extern void initializeFT();
extern void initializeUArea();
...
int mountFileSystem(char *FileName)
{
	...
	{
		char flush[5];
		fscanf(filePointer,"%d%u%d%ld%ld%ld",&type ,&DILB[i].file_size ,&status ,&DILB[i].last_modified ,&DILB[i].last_accessed ,&DILB[i].inode_modified);
		if(type==0)
			DILB[i].type=Free;
		if(type==2)
		...
		if(	DILB[i].type == Directory || DILB[i].type == Free )
