...
int main(void)
{
	...
	char *temp = (char *)malloc(sizeof(char) *80),  *command, **parameter = NULL,*ptr  = NULL; 
	char fileName[20];
	...
	{
		char path[200]="\nhome";
		const char *p=strcat((char *) presentWorkingDirectory(path), " # : ");
		cnt = 0;
		...
		if( strcmp(parameter[0],"mount") == 0 && (cnt == 2) )
		{
			if(mountFlag==0)
			{
		...
		if( strcmp(parameter[0],"new") == 0 && (cnt == 1) )
		{
			if(mountFlag==0)
			{
				initialize();
		...
		{		
			...
			if( strcmp(parameter[0],"pwd") == 0 && (cnt == 1) )
			{
				char path[200]={'\0'};
				printf("\n\tPresent Working Diretory Path : %s",(char *) presentWorkingDirectory(path) );	
			}
			else
			if( strcmp(parameter[0],"rename") == 0 && (cnt == 3) )
			{
			...
			if( strcmp(parameter[0],"cat") == 0 && (cnt == 2) )
				catFile(parameter[1]);
			else
			if( strcmp(parameter[0],"clear") == 0 && (cnt == 1) )
				system("clear");
			else
			if( strcmp(parameter[0],"close") == 0 && (cnt == 2) )
			{
			...
			if( strcmp(parameter[0],"rm") == 0 && (cnt == 2) )
			{
			...
			if( strcmp(parameter[0],"cd") == 0 && (cnt == 2) )
			{
			...
			if( (strcmp(parameter[0],"ls") == 0 ) &&(cnt==1||cnt==2))
			{
				int status=0;
				if(cnt==1)
					status=ls_0();
			...
			if( (strcmp(parameter[0],"touch") == 0 ) &&(cnt==3||cnt==2))
			{
				...
				if(cnt==3)
					status=touch_1(parameter[1], parameter[2]);
				if(status == -1)
					printf("Error occured..!!!\n");
			}
			else
			...
			if( (strcmp(parameter[0],"copy") == 0) && (cnt == 3) )
			{
				int status = 0;
			...
			if( (strcmp(parameter[0],"write") == 0) && (cnt == 2) )
			{
				cnt = 0;
			...
			if( (strcmp(parameter[0],"append") == 0) && (cnt == 2) )
			{
				cnt = 0;
			...
			if( (strcmp(parameter[0],"read") == 0 ) && (cnt == 3) )
			{
				cnt = 0;
			...
			if( (strcmp(parameter[0],"help") == 0 ) && (cnt == 1) )
				help();
			else
				printf("\n\tSyntax error..!! Type \"help\" for correct syntax\n");
		}
		else
		if( ( strcmp(parameter[0],"q") == 0 ) && (cnt == 1) && (mountFlag==0) )
			exit(0);
		else
		if( strcmp(parameter[0],"clear") == 0 && (cnt == 1) )
			system("clear");
